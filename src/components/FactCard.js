  ...
  const placeholderSVG = encodeURIComponent(
    `<svg xmlns='http://www.w3.org/2000/svg' width='800' height='450' viewBox='0 0 800 450'>
      ...
      <text x='50%' y='50%' dominant-baseline='middle' text-anchor='middle'
            fill='#9aa39a' font-family='Arial, sans-serif' font-size='26'>
        No image available
      </text>
    </svg>`
  );
  ...
  return (
    <article className="fact-card">
      ...
      <div className="info">
        ...
        <div className="meta">
          <span>{animal.category || "Unknown category"}</span>
          <span>•</span>
          <span className="small-muted">
            {animal.habitat || "Habitat unknown"}
          </span>
        </div>

        <p className="desc">
          {animal.fact || animal.description || "No description available."}
        </p>
        ...
        <ul className="attributes">
          ...
          {animal.lifespan && (
            <li>
              <strong>Lifespan:</strong> {animal.lifespan} years
            </li>
          )}
          {animal.danger && (
            <li>
              <strong>Danger:</strong> {animal.danger}
            </li>
          )}
          {animal.sourceUrl && (
            <li>
              <strong>Source:</strong>{" "}
              <a href={animal.sourceUrl} target="_blank" rel="noreferrer">
                Wikipedia
              </a>
            </li>
          )}
